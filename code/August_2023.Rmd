...
figure2<-data %>%
  ...
  line_plot(lorenz/spending,
            ...
            add.mean = TRUE,
            line.size = 0.5)
figure2<-figure2 + labs(title="Income (% of Total Income) Earned by the Bottom Half of the Population divided by Public Social Spending (% of GDP)", x="Year",y="Income Share / Spending Share",subtitle = "A decreasing trend implies that income shares (numerator) are decreasing and/or social spending shares (denominator) are increasing.")
figure3<-data %>%
  filter(country !="Canada",country !="France",country !="Mexico",
         country !="Poland",country !="Sweden") %>%
  panel_data(.,id = country,wave = year) %>%
...
```
```{r include=FALSE}
summary(lm(spending~lorenz+as.factor(country)+as.factor(year),data))
```