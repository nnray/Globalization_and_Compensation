...
library(here);library(readxl);library(cowplot);library(tidyverse)
```
...
#data
cmp5<-cmp5 %>%
  rename(country=countryname,date=year) %>%
  select(country,year,anti)
elections<-elections %>%
  select(country,year,enep)
inequality<- inequality %>%
  pivot_longer(2:243,names_to = "country",values_to = "top")
#inequality$country<-str_remove(inequality$country,"^\\S* ")
```
