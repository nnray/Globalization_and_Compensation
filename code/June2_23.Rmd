...
data<-data %>%
  mutate(total_income=(income*households)*lorenz,
         total_spending=spending*gdp,
         total_income_diff=total_income-lag(total_income),
         total_spending_diff=total_spending-lag(total_spending))
```
```{r figures}
figure1<-data %>%
  ...
  line_plot(lorenz,
            overlay = FALSE,
            add.mean = TRUE)
figure1<-figure1 + geom_line(mapping=aes(y=spending))
figure2<-data %>%
  filter(country != "United States") %>%
  panel_data(.,id = country_code,wave = year) %>%
  line_plot(total_income_diff, #take US out?
            overlay = FALSE,
            add.mean = TRUE)
figure2<-figure2 + geom_line(mapping=aes(y=total_spending_diff))
```